...
The image class should be able to:
* read/write images;
* get and set pixel color;
* get a rectangular texture

I use the [CImg](http://cimg.eu) library for read/write `pbm`, `pgm`, `ppm`, and `jpeg` images. The support of `jpeg` depends on the [libjpeg](http://www.ijg.org) library. Support of `png` and `tiff` is possible, but relies on external libraries. The source code can be found in the `src/image` directory of [open3DCV]({{ site.url }}{{ site.baseurl }}/blog/2017/05/3d-vision-lib/). Below are snippets of the implementation
